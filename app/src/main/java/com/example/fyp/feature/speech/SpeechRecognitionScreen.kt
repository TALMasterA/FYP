...
@Composable
private fun SpeechRecognitionContent(
    ...
    Column(
        ...
        // recognize button, recognized text and copy, TTS, translate, etc.
        // all using the callbacks instead of calling the ViewModel directly
        Button(
            onClick = { onRecognize(selectedLanguage) },
            enabled = !AudioRecorder.isRecording,
            modifier = Modifier.fillMaxWidth()
        ) {
            Text(uiText(UiTextKey.AzureRecognizeButton, "Use Azure Recognize (from Mic)"))
        }
        ...
        Button(
            onClick = { onSpeakOriginal(selectedLanguage) },
            enabled = recognizedText.isNotBlank() && !isTtsRunning,
            modifier = Modifier.fillMaxWidth()
        ) {
            Text(
        ...
        Button(
            onClick = { onTranslate(selectedLanguage, selectedTargetLanguage) },
            enabled = recognizedText.isNotBlank(),
            modifier = Modifier.fillMaxWidth()
        ) {
            Text(uiText(UiTextKey.TranslateButton, "Translate"))
        ...
        Button(
            onClick = { onSpeakTranslation(selectedTargetLanguage) },
            enabled = translatedText.isNotBlank() && !isTtsRunning,
            modifier = Modifier.fillMaxWidth()
        ) {
            Text(
